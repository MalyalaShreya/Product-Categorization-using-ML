...
parse a =  do 
        ...
        outh <- openFile (a++"Feature.txt") WriteMode --open a new file in write mode
        mainloop inh outh --calling function mainloop on handles inh and outh
        hClose outh --close the feature.txt file and open in read mode
        ...
        file2 <- openFile (a++"FeatureKeywords.txt") WriteMode --open a new file in write mode
    ...
    do ineof <- hIsEOF inh;
       if ineof 
           then return () --if end of file return
                ...
                {   --getline
                    inpStr <- hGetLine inh;--if the first 8 characters of inpstr =="Feature=" then drop these 8 letters and lower the remaining string and write  into output file 
